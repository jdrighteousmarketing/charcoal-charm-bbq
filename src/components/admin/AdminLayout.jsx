...
export default function AdminLayout() {
  ...
  if (
    isEmployee &&
    !allowedEmployeePaths.some((path) =>
  ...
  <div className="h-16 px-4 flex items-center justify-between"></div>
      ...
      <aside
        className={cn(
          'fixed top-0 left-0 h-full w-64 overflow-hidden bg-card border-r border-border z-50 transition-transform duration-300 pt-[max(env(safe-area-inset-top),32px)] lg:pt-0 lg:translate-x-0',
          sidebarOpen
            ? 'translate-x-0'
            : '-translate-x-full'
        )}
      ...
      {/* Main content */}
      <main className="lg:ml-64 pt-[calc(4rem+max(env(safe-area-inset-top),32px))] lg:pt-0 min-h-screen">
