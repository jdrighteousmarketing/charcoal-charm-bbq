...
export default function CustomerQRCode({ customerIdCode }) {
  ...
  return (
    ...
    >
      <h3 className="font-display font-bold mb-1">My Rewards QR Code</h3>
      <p className="text-xs text-muted-foreground mb-4 text-center">
        Show this to staff to earn or redeem points
      </p>
      <div className="bg-white p-4 rounded-2xl shadow-sm">
        <QRCodeSVG
          value={customerIdCode}
          size={180}
          bgColor="#ffffff"
          fgColor="#1a1a1a"
          level="M"
      ...
      <p className="mt-3 text-xs font-mono text-muted-foreground tracking-widest">{customerIdCode}</p>
    </motion.div>
  );
}