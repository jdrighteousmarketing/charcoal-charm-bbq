...
export default function EmployeeDashboard() {
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: () => base44.entities.PointTransaction.list('-created_date', 10),
  });
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <p className="text-xs font-semibold tracking-widest uppercase text-primary/70 mb-1" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
          Employee Portal
        </p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="border-border/60">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-display font-bold leading-none">{customers.length}</p>
        ...
        <Card className="border-border/60">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-display font-bold leading-none">{transactions.length}</p>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {employeeTools.map(({ path, icon: Icon, label, desc, color, iconColor }, i) => (
            <motion.div
              key={path}
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/admin/customer-directory" className="text-xs text-primary hover:underline">View all</Link>
        </div>
        <Card className="border-border/60">
            ...
            {recentCustomers.length === 0 ? (
              ...
              <div>
                {recentCustomers.map((c, i) => (
                  <div key={c.id} className={`flex items-center justify-between px-4 py-3 ${i !== recentCustomers.length - 1 ? 'border-b border-border/50' : ''}`}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-primary/15 flex items-center justify-center text-xs font-bold text-primary shrink-0">
                        {(c.name || c.email || '?')[0].toUpperCase()}
                      </div>
                      <div>
                        <p className="text-sm font-medium leading-none">{c.name || 'Unknown'}</p>
                        <p className="text-xs text-muted-foreground mt-0.5">{c.email}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-xs font-semibold text-primary">{(c.points_balance || 0).toLocaleString()} pts</p>
                      <p className="text-xs text-muted-foreground">{format(new Date(c.created_date), 'MMM d')}</p>
                    </div>
                  </div>
                ))}
              </div>
